...
function () {
  ...
  _createClass(Player, [{
    ...
    value: function start() {
      ...
      // 音乐要“开始”播放，首先需要“数据”。通过 fetch 去获取数据 data
      fetch('https://ying-king.github.io/data-mock/ying-music/music-list.json').then(function (res) {
        return res.json();
      }) // 返回一个Promise，可以解析成JSON
      .then(function (data) {
        // 获取json数据
        console.log(data);
    ...
    value: function loadSong() {
      ...
      this.$('.header p').innerText = songObj.author + '-' + songObj.album; // 当指定的音频/视频的元数据已加载时，会发生 loadedmetadata 事件
    ...
    value: function setLyrics(lyrics) {
      ...
      .filter(function (str) {
        return str.match(/\[.+?\]/);
      }) //过滤出歌词里边不包含如 [00:30.22] 这种格式的歌词——不是这种格式的话，不是一项有效的歌词
      .forEach(function (line) {
        // 对每一行歌词运用以下逻辑
        var str = line.replace(/\[.+?]/g, ''); // 替换掉前边的 [] 为“空字符”，得到一个纯的字符串

        line.match(/\[.+?]/g).forEach(function (t) {
          // 对匹配到的每一行的“时间”运用以下逻辑
          // 把时间点的“中括号”去掉，得到一个单纯的时间，如 00:30.22
          t = t.replace(/[\[\]]/g, ''); // 将时间统一全部变成“毫秒

          var milliseconds = parseInt(t.slice(0, 2)) * 60 * 1000 + parseInt(t.slice(3, 5)) * 1000 + parseInt(t.slice(6)); // 然后创建一个新的数组，数组里是时间“毫秒数”和歌词“字符串”,放入 lyricsArr
      ...
      lyricsArr.filter(function (line) {
        ...
        // 排序好后，对里边的每一条运用以下逻辑
        var node = document.createElement('p'); // 在页面创建一个 p 标签
    ...
    key: "bind",
    value: function bind() {
      // 当用户点击任何一个按钮时，播放器的运行逻辑都是：先“缓存”对应歌曲的信息，然后“播放”该歌曲
      var self = this; // 操作按钮

      var btnPlayPause = this.$('.btn-play-pause');
      var btnPre = this.$('.btn-pre');
      var btnNext = this.$('.btn-next');
      var btnLoad = this.$('.btn-load');
      var btnLike = this.$('.btn-like'); // 进度条相关

      var bar = this.$('.bar');
      var progress = this.$('.bar .progress');
      var control = this.$('.bar .control');
      var touchPoint = 0; // 初始化触摸起点

      var crlLeft = 0; // 控件离容器最左方的距离
      // 播放、停止按钮
      ...
      control.addEventListener('touchmove', function (e) {
        var touch = e.changedTouches[0];
        var diffX = touch.clientX - touchPoint;
        var crlLeftStyle = crlLeft + diffX;
        var maxX = bar.offsetWidth - 8;
      ...
      }, false); // 点击bar，让进度条直接到达这个位置

      bar.onclick = function (e) {
        var barLeft = e.clientX - this.offsetLeft;
    ...
    value: function locateLyric() {
      ...
      var nextLineTime = this.lyricsArr[this.lyricIndex + 1][0]; // 播放歌曲的时间大于 DOM 元素
    ...
    value: function setLyricToCenter(node) {
      // 将播放的歌词放到“歌词面板”中间并“高亮”
      console.log(node); //偏移的高度:歌词元素到顶点高度 - 歌词面板高度/2

      var translateY = node.offsetTop - this.$('.panel-lyrics').offsetHeight / 2;
      translateY = translateY > 0 ? translateY : 0; // 把歌词放到“歌词面板”的中央

      this.$('.panel-lyrics .container').style.transform = "translateY(-".concat(translateY, "px)"); // 歌词“高亮”的设置
...
},{"./icons":"src/js/icons.js","./swiper":"src/js/swiper.js"}],"node_modules/_parcel-bundler@1.12.3@parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
